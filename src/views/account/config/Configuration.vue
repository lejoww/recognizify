            ...
            <div class="configurationView">
                <div class="configurationAlignment">
                    ...
                    <div class="tab-content" id="myTabContent">
                        ...
                        <div class="tab-pane fade" id="security" role="tabpanel" aria-labelledby="security-tab"><br>
                            <h4>Configura la seguridad de tu cuenta</h4>
                            <div class="form-group">
                                <label>Selecciona el nivel de privacidad de tu cuenta</label>
                                <select class="form-control" style="border: 1px solid #e0e0e0;" @change="setConfigurationAccountMode" id="mode">
                                    <option selected>Selecciona un modo</option>
        ...
        methods: {
            ...
            normalizeString: function() {
                ...
                for(let i = 0, j = this.name.length; i < j; i++) {
                    ...
                    if(mapping.hasOwnProperty(this.name.charAt( i ))) ret.push(mapping[ c ]);
                    else ret.push(c);
                }      
                return ret.join('');
            },
            setConfigurationAccountMode: function(){
                firebase.firestore()
                .collection('users')
                .doc(firebase.auth().currentUser.uid)
                .update({
                    visibilityMode: document.getElementById('mode').value
                })
                .then(() => {
                    let alert = document.getElementById('alert-success')
                    alert.style.display = 'block'
